...
| WebSocket handshake vs auth | **Corrected 18 Sep by captures-ws.md** | `[live]` the upgrade returns 101 with IDENTICAL headers whatever the credential, and the socket is silent until the CLIENT speaks. The 16 Sep reading ("the auth error arrives as the first frame" / "with a bad key the socket stays silent") was an artefact of whether a message had been sent: both probes were the same server, answering or not answering a `create`. A bad key gets top-level `error` code 7 and a server CLOSE 1000 in the same millisecond, ~320 ms after the first client frame; no credential gets code 16 the same way; `?key=` is read as NO credential. Only `Authorization: Basic <key>` works. 101-then-silence is a HEALTHY idle socket (75 s, zero frames, still usable afterwards) — see captures-ws.md probes 1-3 |
...
| WebSocket success path (context lifecycle, usage cadence) | **Resolved 18 Sep by captures-ws.md** | full transcript, sizes and timings in captures-ws.md 1.2; `RECOGNITION_USAGE` does not exist on the wire (see the STT unit row) |
...
| Session lifecycle / keepalive | LK-TTS `[source: tts.py:259-266 connect; :443-445 receive(timeout=60.0); :592-600 stale-context sweep at 120 s]`; **confirmed 18 Sep by captures-ws.md** | no ping/pong IN EITHER DIRECTION: the server sent no PING and no close in 75 s of idle, and the context was still usable afterwards. Liveness on this provider is entirely client-driven, which is why the gateway's own `idle` budget (clocks.Budgets.idle, 600 s for `tts_session`) is the only thing that reclaims an abandoned session | **Needs new transport** | — | `Context not found` (code 5) is benign |
...
| STT unit | PRICE; **corrected 18 Sep by captures-ws.md** | **per hour of audio**: $0.15/hr on-demand, $0.10/hr Creator+. The "`RECOGNITION_USAGE` every 5 s" in the 16 Sep sweep is the PLUGIN's own client-side event computed from audio it pushed (`stt.py:368-376`), not a wire frame. On the wire there is ONE `{"result":{"usage":{"transcribedAudioMs","modelId"}}}` frame, after `closeStream` — 1,500 ms for 1.63 s of speech, 3,150 ms for 7.6 s streamed, so it is the provider's own count (speech plus the end-of-turn tail) and NOT bytes ÷ rate. A session that ends without `closeStream` has no meter at all | **Needs new accounting** + WS | captures-ws.md 1.1 probes 6/6b | A gateway session with no `closeStream` records `estimated` seconds and says so in `cost_notes` |
...
| `GET /tts/v1/voice:streamBidirectional` with WebSocket upgrade headers, Basic fake | **101 Switching Protocols**, then no frame for 15 s | **Corrected 18 Sep by captures-ws.md**: the silence is because nothing was SENT. A `create` on this socket earns `{"error":{"code":7,…}}` + server CLOSE 1000 ~320 ms later |
| `GET /stt/v1/transcribe:streamBidirectional` with upgrade headers, no credential | **101**, then first text frame | **Corrected 18 Sep by captures-ws.md**: the frame followed the probe's own first message, not the upgrade. The body is `{"error":{"code":16,"message":"authentication is required"}}` and a server CLOSE 1000 follows it immediately |
