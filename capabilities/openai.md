...
| `POST /v1/responses` | …/api-reference/responses/create | **Unsupported** (501) | `app.py:286` `UNIMPLEMENTED_ROUTES`; docstring: no surface, `response.failed` ending not modelled | Docs now say "Responses API is recommended over Chat Completions" for reasoning models |
...
| `/v1/images/*` (generations/edits/variations) | api-reference nav | **Unsupported** (404) | `live_smoke.md:19` | Image *generation*; vision *input* is a chat feature (see §2) |
...
| Reasoning summaries / `encrypted_content` | reasoning | **Unsupported** | Responses-only → 501 | — |
...
| `web_search_options` / built-in hosted tools | chat/create; responses | **Passthrough** (chat) / **Unsupported** (Responses tools) | — | Web-search calls carry extra per-call pricing not in `ModelSpec` → cost blind |
| `moderation` param / `moderation` response field | chat/create | **Passthrough** | — | New; unknown keys ignored |
| `background: true`, `previous_response_id`, `conversation` | responses/create | **Unsupported** | 501 | Responses-only |
...
| Responses semantic events (`response.*`, `response.failed`, `response.incomplete`) | responses/create | **Unsupported** | 501; `openai.py:257-262` notes `response.failed` must be forwarded when built | — |
...
3. **`/v1/responses` is 501** while OpenAI now steers reasoning/agentic use (hosted tools, `previous_response_id`, encrypted reasoning, background mode) to it. Any tcg agent built on the Responses SDK cannot use the gateway.
...
- Current default models in docs are `gpt-6-astra` / `gpt-5.6-*`; the catalog has none of them, and `live/smoke.py`'s `gpt-5-nano` row is not in the shipped catalog either.
- Structured-outputs docs list the `refusal` field and `incomplete_details.reason` (Responses) — the chat surface treats `refusal` as progress but nothing records a refusal outcome.
