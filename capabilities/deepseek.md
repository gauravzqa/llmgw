...
Repo facts that drive most rows: DeepSeek is `ProviderConn(kind="openai", base_url="https://api.deepseek.com")` (`catalog.py:174-180`) and rides the OpenAI surface; the request body is never re-serialised, only `model` is rewritten (`surfaces/openai.py:59-86`, `live_smoke.md` "everything is byte-faithful passthrough"). Routes: `/v1/chat/completions`, `/anthropic/v1/messages` only; `/v1/responses` explicitly unimplemented (`server/app.py:238-239, 286`).
...
| Responses API `POST /responses` (stateless, no `previous_response_id`, `output_config.effort`) | `/guides/responses_api` | **Unsupported** | `app.py:286 UNIMPLEMENTED_ROUTES=("/v1/responses",)` | Same gap as for OpenAI. |
| `/beta` base URL (prefix completion, strict tools) | `/guides/chat_prefix_completion`, `/guides/tool_calls` | **Unsupported** | no `deepseek-beta` provider; `join_url` with base `…/beta` would emit `/beta/v1/chat/completions` — unknown whether DeepSeek accepts the `/v1` inside `/beta` | Test: `curl https://api.deepseek.com/beta/v1/chat/completions`. If 404, `join_url` needs a per-provider "path prefix" notion. |
...
| Base URL `https://api.deepseek.com` (and `/v1`, `/beta`, `/anthropic`) | home, guides | `/` **Supported**; `/beta`, `/anthropic` **Unsupported** (no provider rows) | `catalog.py:177` | |
...
9. **`/beta` features (strict tools, prefix completion) and FIM are unreachable.** Needs a second provider row and a `join_url` that tolerates `/beta/v1/…` or a path-prefix field.
...
- **Base URLs:** `/anthropic` and `/beta` exist upstream; repo knows only the root.
